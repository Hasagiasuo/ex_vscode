...
OfferWinAdd::~OfferWinAdd() {
  delete this->path_img;
  delete this->title;
...
: wxFrame(nullptr, wxID_ANY, "", wxPoint(wxDisplay().GetGeometry().GetSize().x / 2 - 300, wxDisplay().GetGeometry().GetSize().y / 2 - 300), wxSize(600, 600), wxBORDER_NONE){
  ...
  this->title = new wxTextCtrl(this, wxID_ANY, "", wxPoint(20, 300), wxSize(100, 20), wxBORDER_SUNKEN);
  ...
  this->description = new wxTextCtrl(this, wxID_ANY, "" , wxPoint(20, 350), wxSize(200, 40), wxBORDER_SUNKEN);
...
void OfferWinAdd::draw_img(wxPaintEvent& ev) {
  wxPaintDC dc(this);
  if(this->choice_img) {
    dc.DrawBitmap(*this->choice_img, wxPoint(20, 20));
